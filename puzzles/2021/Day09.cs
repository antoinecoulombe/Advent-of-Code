...
namespace aoc.puzzles._2021
{
  class Day09 : Day
  {
    ...
    private List<Point> GetSides(Point p, Point? from)
    {
      ...
      if (p.X > 0 && ValidPoint(p.X - 1, p.Y) && NotFrom(p.X - 1, p.Y, from))
        sides.Add(new Point(p.X - 1, p.Y));
      if (p.X < numbers[p.Y].Count() - 1 && ValidPoint(p.X + 1, p.Y) && NotFrom(p.X + 1, p.Y, from))
        sides.Add(new Point(p.X + 1, p.Y));
      if (p.Y > 0 && ValidPoint(p.X, p.Y - 1) && NotFrom(p.X, p.Y - 1, from))
        sides.Add(new Point(p.X, p.Y - 1));
      if (p.Y < numbers.Count() - 1 && ValidPoint(p.X, p.Y + 1) && NotFrom(p.X, p.Y + 1, from))
        sides.Add(new Point(p.X, p.Y + 1));
      return sides;
    }
