...
namespace aoc.puzzles
{
  abstract class Day
  {
    ...
    private static string baseFolder = projectFolderName + sep;
    ...
    protected static string GetRessourcePath(string fileName = null)
    {
      try
      {
        ...
        return $"{path}{codeFolderName}{sep}{caller[2].Substring(1)}{sep}ressources{sep}day_{caller[3].Substring(3)}{sep}{fileName}";
      }
